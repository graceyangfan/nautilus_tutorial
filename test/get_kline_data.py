...
def test_imbalance_corr(df):
    ...
    IS_Corr = df_train.pearson_corr()[0,"label"]
    OS_Corr = df_test.pearson_corr()[0,"label"]
...
def select_params(df,njobs=1):
    ...
    params=[]
    for period in np.arange(250,300,100):
        for threshold in np.arange(8,9,0.1):
            for value in np.arange(5,10,20):
                params.append({
                    "df":df,
                    "period":period,
                    "threshold":threshold,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--filename', default="/tmp/BTCUSDT*.csv")
