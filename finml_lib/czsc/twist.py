...
class Twist:
    ...
    def trend_break_signal(
        self,
        base_line_type = LineType.XD,
        line_num: int = 5,
    ):
        try:
            up_model, down_model = self.trend_channel_model(base_line_type,line_num)
        except:
            return 0 
        ...
        if 0 < up_model.coef_ < down_model.coef_ and predict_down < predict_up and \
            self.newbars[-2].close > predict_down and self.newbars[-1].close < predict_down:
            return  -1
        if 0 > down_model.coef_ > up_model.coef_ and predict_down < predict_up and \
            self.newbars[-2].close < predict_up and self.newbars[-1].close > predict_up:
            return  1 
