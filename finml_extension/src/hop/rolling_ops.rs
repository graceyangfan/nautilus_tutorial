...
#[polars_expr(output_type=Float64)]
pub fn pl_rolling_idxmin(inputs: &[Series], kwargs: GroupByRollingKwargs) ->PolarsResult<Series> 
{
    let out_name = "idxmin";    let mut result: Vec<Series> = Vec::new();
    // Add index series
    let index_series = Series::new("index", (0..inputs[0].len() as i64).collect::<Vec<_>>());
    result.push(index_series);
    // Add x series
