...
class DoubleMaConfig(StrategyConfig):
    """
    Configuration for ``DoubleMa`` instances.
...
class DoubleMa(Strategy):
    ...
    def __init__(self, config: DoubleMaConfig):
        ...
        self.fast_window = config.fast_ema_period
        self.slow_window = config.slow_ema_period 
        ...
        self.close_array = deque(maxlen=1440)
    ...
    def on_bar(self, bar: Bar):
        ...
        self.close_array.append(bar.close.as_double())
        if len(self.close_array) < 1440:
            return  

        fast_ma = talib.EMA(np.array(self.close_array),self.fast_window)
        self.fast_ma0 = fast_ma[-1]
        self.fast_ma1 = fast_ma[-2]
        slow_ma = talib.EMA(np.array(self.close_array),self.slow_window)
        ...
        if cross_over:
            if self.portfolio.is_flat(self.instrument_id):
                self.buy(bar.close.as_double(), self.trade_size)
            ...
            if self.portfolio.is_flat(self.instrument_id):
                self.sell(bar.close.as_double(), self.trade_size)
